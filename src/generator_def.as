                         ...
                         x: *code.StaticSlot): *code.Handle ->
{
    ...
    let init = llvm.LLVMGetInitializer(x.handle);
                      ...
                      x: *code.Function) ->
{
    ...
    let entry = llvm.LLVMAppendBasicBlock(x.handle, "");

    # Remember the insert block.
    let mut cur_block: *llvm.LLVMOpaqueBasicBlock;
    ...
    let mut ret_type_target: *code.Handle = type_.return_type;
        ...
        if code.isnil(res)
        {
            ...
            else
            {
                ...
                errors.libc.fprintf(errors.libc.stderr,
                               "mismatched types: expected '%s' but found nothing",
                               s_typename.data());
                errors.end();
...
# -----------------------------------------------------------------------------
let generate_attached_function(mut g: generator_.Generator, qname: str,
...
# -----------------------------------------------------------------------------
let is_assignable(mut g: generator_.Generator, handle: *code.Handle): bool ->
{
    if handle._tag == code.TAG_STATIC_SLOT { return true; }
    else if handle._tag == code.TAG_LOCAL_SLOT {
        return true;
    }
    else if handle._tag == code.TAG_VALUE {
        let val: *code.Value = handle._object as *code.Value;
        return val.category == code.VC_LVALUE;
        0; #HACK
    }
...
# -----------------------------------------------------------------------------
let to_value(mut g: generator_.Generator,
             handle: *code.Handle,
             category: int,
             static_: bool): *code.Handle ->
{
    if handle._tag == code.TAG_STATIC_SLOT
    {
        ...
        if category == code.VC_RVALUE
        {
            if static_
            {
                ...
                return generate_static_slot(g, slot.name.data() as str, slot);
                0; # HACK
            }
            else if slot.type_._tag == code.TAG_ARRAY_TYPE
            {
                ...
                # Wrap it in a handle.
                return code.make_value(slot.type_, category, slot.handle);
                0; # HACK
            }
            else
            {
                ...
                let mut val: *llvm.LLVMOpaqueValue;
                val = llvm.LLVMBuildLoad(g.irb, slot.handle, "");
    ...
    if handle._tag == code.TAG_LOCAL_SLOT
    {
    ...
    if handle._tag == code.TAG_VALUE
    {
        ...
        {
            if obj.type_._tag == code.TAG_ARRAY_TYPE
            {
                # We have an array type.
                # Wrap it in a handle.
                return code.make_value(obj.type_, category, obj.handle);
                0; #HACK!
            }
            else
            {
                ...
                let mut val: *llvm.LLVMOpaqueValue;
                val = llvm.LLVMBuildLoad(g.irb, obj.handle, "");
    ...
    if handle._tag == code.TAG_FUNCTION {
        ...
        return code.make_value(fn_type_handle, code.VC_LVALUE, fn.handle);
    };

    if handle._tag == code.TAG_ATTACHED_FUNCTION {
        # Get the function handle.
    ...
    if handle._tag == code.TAG_EXTERN_STATIC {
        # Get the handle.
        ...
        # Create a handle of the function.
        return code.make_value(type_handle, code.VC_LVALUE, val);
    };

    if handle._tag == code.TAG_EXTERN_FUNC {
        # Get the function handle.
    ...
    if handle._tag == code.TAG_TUPLE {
        ...
        {
            # Get the handle out.
            let han: *code.Handle = tuple_.handles.get_ptr(i) as *code.Handle;

            # Resolve the type of the element expression.
            let el_type: *code.Handle = tuple_type.elements.get_ptr(i) as *code.Handle;
        ...
        if category == code.VC_LVALUE {
            ...
            return code.make_value(type_, code.VC_LVALUE, slot_val);
        } else {
            # Wrap and return the value.
            return code.make_value(type_, code.VC_RVALUE, val);
        };
    };
...
# Assignment
# -----------------------------------------------------------------------------
let assign(mut g: generator_.Generator,
           target: *code.Handle,
           lhs: *code.Handle,
           rhs: *code.Handle) -> {
        ...
        (lhs._tag == code.TAG_EXTERN_STATIC))
    {
        ...
        if not slot.mutable {
            ...
            errors.begin_error();
            errors.libc.fprintf(errors.libc.stderr,
                           "re-assignment to immutable local slot");
            errors.end();
            return;
        };
        ...
        while i as uint < tuple_.handles.size {
            ...
            let val: *llvm.LLVMOpaqueValue =
                llvm.LLVMBuildStructGEP(
                    g.irb, rhs_val.handle, i as uint32, "");

            # Get the element handle.
            let el: *code.Handle = tuple_.handles.get_ptr(i) as *code.Handle;
