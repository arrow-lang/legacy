...
# -----------------------------------------------------------------------------
let resolve_t(g: *mut generator_.Generator, node: *ast.Node,
              target: *code.Handle): *code.Handle ->
{
...
# Resolve an arbitrary targeted type expression from an AST node.
# -----------------------------------------------------------------------------
let resolve_st(g: *mut generator_.Generator, node: *ast.Node,
               scope: *code.Scope,
               target: *code.Handle): *code.Handle ->
{
...
# Resolve an arbitrary targeted type expression from an AST node.
# -----------------------------------------------------------------------------
let resolve_s(g: *mut generator_.Generator, node: *ast.Node,
...
# Resolve an arbitrary targeted type expression from an AST node.
# -----------------------------------------------------------------------------
let resolve_in_t(g: *mut generator_.Generator, node: *ast.Node,
                 ns: *list.List,
                 target: *code.Handle): *code.Handle ->
{
...
# -----------------------------------------------------------------------------
let resolve(g: *mut generator_.Generator, node: *ast.Node): *code.Handle ->
{
...
# Resolve an arbitrary type expression from an AST node.
# -----------------------------------------------------------------------------
let resolve_in(g: *mut generator_.Generator, node: *ast.Node, ns: *list.List,
               scope: *code.Scope, target: *code.Handle): *code.Handle ->
{
    # Get the type resolution func.
    let res_fn = g.type_resolvers[node.tag];

    # TODO: Bail if we don't have a resolver.
    # if (res_fn as *int8) as uint == 0 {
    #     errors.begin_error();
